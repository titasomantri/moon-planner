...
if ($a=="belum diterima") 
{
echo "<script>alert('Customer belum Mengkonfirmasi bahwa Dia sudah menerima barang.'); window.location = 'index.html?page=laporan'</script>";
}
else{
mysqli_query($conn,"DELETE FROM chekout WHERE id_chekout='$kd'");
mysqli_query($conn,"DELETE FROM keranjang where id_pembeli='$id'");
header("location:index.html?page=laporan");}
 ?>